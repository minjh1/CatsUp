...
export class ReplyPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, private http: Http) {
    this.replyType = this.navParams.get("replyType");
  ...
  getFeedReplies(limit, offset, seq) {
    ...
    this.http.post(this.serverURL + '/getFeedReplies', JSON.stringify(body), { headers: headers })
      .map(res => res.json())
      .subscribe(data => {
        for (let i = 0; i < data.length; i++) {
          this.replies.push(new Reply(data[i].reply_seq, this.serverURL+data[i].imgUrl, data[i].user_seq, data[i].nickname,
